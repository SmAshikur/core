$(function ($) {
    ...
    $(document).ready(function () {
       ...
       // $('.product-details-slider .item').zoom();
        ...
        function cartSubmit(item_id) {
            console.log(item_key);
            getData(1, item_key,item_id);
        };

        function getData(status = 0, check = 0, item_key = 0, add_type = 0) {
            let itemId;
            let type;
            if (check != 0) {
                itemId = check;
            ...
            if (status == 1) {
                let addToCartUrl = `${mainurl}/product/add/cart?item_id=${itemId}`;
                $.ajax({
                    type: "GET",
                    url: addToCartUrl,
                    success: function (data) {
