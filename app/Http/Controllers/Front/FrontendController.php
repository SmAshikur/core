...
// -------------------------------- HOME ----------------------------------------
	...
	public function index()
	{
        ...
        $setting = Setting::first();
            ...
            $courses = Course::latest()->where('category','mother-father-traning')->where('status',1)->limit(3)->get(['title','slug','sub_title','price','time','description']);
            $teachers_tring = Course::latest()->where('category','teacher-traning')->where('status',1)->limit(3)->get(['title','slug','sub_title','price','time','description']);
            $our_workshops = Course::latest()->where('category','our-workshops')->where('status',1)->limit(3)->get(['id','title','slug','sub_title','price','time','description']);
            return view('front.index',[
                ...
                'courses'=>$courses,
                'teachers_tring'=>$teachers_tring,
                'our_workshops'=>$our_workshops,
            ]);
	}
...
//----------------------Teachers Traning
    ...
    public function teachersTraning($item){
        return view('front.course.course',[
            'courses'    =>Course::where('category',$item)->where('status',1)->paginate(24),
        ]);
    }

    public function singleCourse($slug){
        return view('front.course.single_course',[
    ...
    public function courseOrder(Request $request){
        $data=$request->all();
        $data['order_id']='#'.rand(111111,999999);
        $data['status'] = 0;
        $order = CourseOrder::create($data);
        return view('front.course.order',[
            'order'=> $order,
