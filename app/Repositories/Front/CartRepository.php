...
class CartRepository
{
    ...
    public function workshopStore($request)
    {
        ...
        //$qty = is_numeric($qty) ? $qty : 1;
        $cart = Session::get('cart');
        $item = Course::where('id',$input['id'])->select('id','category','title','slug','price','image')->first();
        //$single = isset($request->type) ? ($request->type == '1' ? 1 : 0 ) : 0;
        if(Session::has('cart')){
            //if($item->item_type == 'digital' || $item->item_type == 'license'){
                $check = array_key_exists($input['item_id'],Session::get('cart'));
                ...
                if($check){
                    ...
                    if(array_key_exists($input['id'].'-',Session::get('cart'))){
                        return __('Product all-ready added');
                    }
                }
            //}
